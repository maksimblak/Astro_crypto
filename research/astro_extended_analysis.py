...
def analyze_extended_combos(pdf, bdf):
    ...
    print(f"\n  Найдено {len(results)} комбинаций после фильтра по ratio")
    print(f"  {'Комбинация':<50} {'N':>4} {'Разв%':>7} {'Base%':>7} {'Ratio':>7} {'p-value':>10} {'q-value':>10}")
    print("  " + "-" * 102)
    for r in results[:30]:
        q_value = r.get("q_value", 1.0)
        sig = " **" if q_value < 0.01 else " *" if q_value < 0.05 else ""
        print(f"  {r['combo']:<50} {r['pivots']:>4} {r['pivot_pct']:>5.1f}% "
              f"{r['base_pct']:>5.1f}% {r['ratio']:>6.2f}x {r['p_value']:>10.4f} {q_value:>10.4f}{sig}")
...
def plot_extended_results(pdf, bdf, combo_results):
    ...
    if combo_results:
        ...
        p_colors = ["#4CAF50" if r.get("q_value", 1.0) < 0.05 else "#FFC107" if r["p_value"] < 0.1 else "#F44336" for r in top]
        ...
        ax.set_title("Топ-12 комбинаций факторов (цвет = FDR)", fontweight="bold")
...
def print_verdict(pdf, bdf, combo_results):
    ...
    for r in combo_results:
        if r.get("q_value", 1.0) < 0.05:
            findings.append((r["combo"], r["pivot_pct"], r["base_pct"], r.get("q_value", r["p_value"]), "combo"))
