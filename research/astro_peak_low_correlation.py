...
SLOW_PLANETS = ["Юпитер", "Сатурн"]
PERSONAL_PLANETS = ["Меркурий", "Венера", "Марс"]
ALL_PLANETS = ["Солнце", "Луна"] + PERSONAL_PLANETS + SLOW_PLANETS


# ── Утилиты ──────────────────────────────────────────────────────────

def _lon_deg(planet_name, d):
    return planet_lon_deg(planet_name, d)


def _is_retrograde(planet_name, d):
    if isinstance(d, datetime):
        d_prev = d - timedelta(days=1)
    else:
        d_prev = d - timedelta(days=1)
    return _is_retrograde_3arg(planet_name, d, d_prev)


def _near_station(planet_name, date, window=STATION_WINDOW):
...
def compute_astro(date: datetime) -> dict:
    ...
    # Ретроградность
    retro = {}
    for name in PERSONAL_PLANETS + SLOW_PLANETS:
        retro[name] = _is_retrograde(name, d)
    retro_count = sum(retro.values())

    # Станции
    stations = {}
    for name in PERSONAL_PLANETS:
        stations[name] = _near_station(name, date)
