"""Market regime calculation service.

Moved from dashboard/market_regime.py to decouple business logic
from the dashboard layer. Backend routers import from here.
"""
...
def _load_thresholds():
    """Load all thresholds from btc_config table, fallback to defaults."""
    try:
        from backend.services.config_service import get_thresholds
        direction = get_thresholds("direction")
        stress = get_thresholds("stress")
        context = get_thresholds("context")
        regime = get_thresholds("regime")
    except Exception:
        logger.warning("Config table unavailable, using hardcoded defaults", exc_info=True)
        from backend.services.config_service import DEFAULTS
        direction = {k: float(v) for k, v in DEFAULTS["direction"].items()}
        stress = {k: float(v) for k, v in DEFAULTS["stress"].items()}
        context = {k: float(v) for k, v in DEFAULTS["context"].items()}
        regime = {k: float(v) for k, v in DEFAULTS["regime"].items()}
    return direction, stress, context, regime


# Lazy-loaded thresholds (refreshed per build_regime_payload call)
_direction_t: dict = {}
_stress_t: dict = {}
_context_t: dict = {}
_regime_t: dict = {}
...
def _refresh_thresholds():
    global _direction_t, _stress_t, _context_t, _regime_t
    _direction_t, _stress_t, _context_t, _regime_t = _load_thresholds()
...
def _classify_regime(metrics, direction_score, stress_score):
    ...
    if direction_score >= _regime_t["strong_bull"]:
        if (drawdown is not None and drawdown <= _regime_t["recovery_drawdown"]) or (close_vs_200 is not None and close_vs_200 <= 0):
            return {
                "code": "recovery",
    ...
    if direction_score >= _regime_t["mild_bull"]:
        if drawdown is not None and drawdown <= _direction_t["drawdown_shallow"]:
            return {
                "code": "recovery",
                "label": "Восстановление",
                "bias": "risk-on",
    ...
    if direction_score <= _regime_t["strong_bear"]:
        return {
            "code": "bear",
    ...
    if direction_score <= _regime_t["mild_bear"]:
        return {
            "code": "bear",
            "label": "Медвежий тренд",
            "bias": "risk-off",
    ...
    if direction_score <= _regime_t["distribution"]:
        return {
            "code": "distribution",
            "label": "Распределение",
            "bias": "risk-off",
...
def _build_direction_signals(metrics):
    ...
    if close_vs_200 is None:
        close_impact = 0
    elif close_vs_200 > _direction_t["close_vs_200_strong"]:
        close_impact = 3
    elif close_vs_200 > 0:
        close_impact = 2
    elif close_vs_200 <= -_direction_t["close_vs_200_strong"]:
    ...
    if sma50_vs_200 is None:
        cross_impact = 0
    elif sma50_vs_200 > _direction_t["sma50_vs_200_strong"]:
        cross_impact = 2
    elif sma50_vs_200 > 0:
        cross_impact = 1
    elif sma50_vs_200 <= -_direction_t["sma50_vs_200_strong"]:
    ...
    if mom20 is None:
        mom20_impact = 0
    elif mom20 > _direction_t["momentum_20_strong"]:
        mom20_impact = 2
    elif mom20 > _direction_t["momentum_20_mild"]:
        mom20_impact = 1
    elif mom20 < -_direction_t["momentum_20_strong"]:
        mom20_impact = -2
    elif mom20 < -_direction_t["momentum_20_mild"]:
    ...
    if mom90 is None:
        mom90_impact = 0
    elif mom90 > _direction_t["momentum_90_very_strong"]:
        mom90_impact = 5
    elif mom90 > _direction_t["momentum_90_strong"]:
        mom90_impact = 4
    elif mom90 > 0:
        mom90_impact = 2
    elif mom90 < -_direction_t["momentum_90_very_strong"]:
    ...
    if drawdown is None:
        drawdown_impact = 0
    elif drawdown >= _direction_t["drawdown_shallow"]:
        drawdown_impact = 3
    elif drawdown > _direction_t["drawdown_moderate"]:
        drawdown_impact = 1
    elif drawdown <= _direction_t["drawdown_extreme"]:
        drawdown_impact = -4
    elif drawdown <= _direction_t["drawdown_deep"]:
        drawdown_impact = -3
    elif drawdown <= _direction_t["drawdown_moderate"]:
        drawdown_impact = -2
    else:
    ...
    if volume_z is None or mom20 is None or volume_z < _direction_t["volume_spike_z"]:
        volume_impact = 0
    elif mom20 > 0:
        volume_impact = 1
    elif mom20 < 0:
...
def _build_stress_signals(metrics):
    ...
    if amihud_z is None:
        amihud_points = 0
    elif amihud_z >= _stress_t["amihud_z_high"]:
        amihud_points = 2
    elif amihud_z >= _stress_t["amihud_z_elevated"]:
        amihud_points = 1
    else:
        amihud_points = 0
    ...
    if range_compression is None:
        range_points = 0
    elif range_compression >= _stress_t["range_expansion_high"]:
        range_points = 2
    elif range_compression >= _stress_t["range_expansion_elevated"]:
        range_points = 1
    else:
        range_points = 0
    ...
    volume_z = metrics["volume_z_20"]
    volume_points = 1 if volume_z is not None and abs(volume_z) >= _stress_t["volume_shock_z"] else 0

    mom20 = metrics["momentum_20"]
    momentum_points = 1 if mom20 is not None and mom20 <= -_direction_t["momentum_20_strong"] else 0

    dxy_return = metrics["dxy_return_20d"]
    if dxy_return is None:
        dxy_points = 0
    elif dxy_return >= _context_t["dxy_return_high"]:
        dxy_points = 2
    elif dxy_return >= _context_t["dxy_return_elevated"]:
        dxy_points = 1
    else:
        dxy_points = 0
    ...
    if us10y_change is None:
        us10y_points = 0
    elif us10y_change >= _context_t["us10y_change_high_bps"]:
        us10y_points = 2
    elif us10y_change >= _context_t["us10y_change_elevated_bps"]:
        us10y_points = 1
    else:
        us10y_points = 0
...
def _build_context_signals(metrics):
    ...
    if wiki_z is None:
        wiki_score = 0
    elif wiki_z >= _context_t["wiki_z_high"]:
        wiki_score = 2
    elif wiki_z <= _context_t["wiki_z_low"]:
        wiki_score = -2
    else:
        wiki_score = 0
    ...
    if fear_greed is None:
        fear_score = 0
    elif fear_greed >= _context_t["fear_greed_greedy"]:
        fear_score = 3
    elif fear_greed <= _context_t["fear_greed_fearful"]:
        fear_score = -3
    else:
        fear_score = 0
    ...
    if funding_z is None:
        funding_score = 0
    elif funding_z >= _context_t["funding_z_high"]:
        funding_score = 1
    elif funding_z <= _context_t["funding_z_low"]:
        funding_score = -1
    else:
        funding_score = 0
    ...
    if perp_premium is None:
        premium_score = 0
    elif perp_premium >= _context_t["perp_premium_high"]:
        premium_score = 2
    elif perp_premium <= _context_t["perp_premium_low"]:
        premium_score = -2
    else:
        premium_score = 0
    ...
    if oi_z is None:
        oi_score = 0
    elif oi_z <= _context_t["oi_z_low"]:
        oi_score = 1
    elif oi_z >= _context_t["oi_z_high"]:
        oi_score = -1
    else:
        oi_score = 0
    ...
    if active_addresses_z is None:
        addresses_score = 0
    elif active_addresses_z >= _context_t["addresses_z_high"]:
        addresses_score = 2
    elif active_addresses_z <= _context_t["addresses_z_low"]:
        addresses_score = -2
    else:
        addresses_score = 0
    ...
    elif funding_bias > 0 and funding_divergence >= _context_t["funding_divergence_strong"]:
        funding_divergence_score = 2
    elif funding_bias > 0 and funding_divergence >= _context_t["funding_divergence_mild"]:
    ...
    if oi_state == "short_build":
        ...
        oi_state_score = 1 if oi_delta_z is None or oi_delta_z <= _context_t["oi_delta_z_elevated"] else 0
